#!/usr/bin/env python3
"""
Train image models (NumPy from-scratch + PyTorch) for any registered image dataset.

End-to-end pipeline:
  1. Check for prepared data → if missing, preprocess from raw images
  2. Train custom NumPy model (using Trainer from src/training/)
  3. Train PyTorch model (using AudioClassifier — architecture is modality-agnostic)
  4. Generate t-SNE embeddings for both engines
  5. Save all artifacts to registry-defined paths

Artifacts saved per dataset:
  - models/image/<dataset>/model.npz                     (NumPy weights)
  - models/image/<dataset>/pytorch_model.pt              (PyTorch weights)
  - models/image/<dataset>/norm_stats.npz                (mean/std for inference)
  - data/prepared/image/<dataset>/prepared_data.npz      (train/val/test splits)
  - results/metrics/image/<dataset>/training_history.npz
  - results/metrics/image/<dataset>/pytorch_training_history.npz
  - results/metrics/image/<dataset>/tsne_data.npz
  - results/metrics/image/<dataset>/pytorch_tsne_data.npz

Usage:
    python train_image.py                                 # Default: image/wildlife
    python train_image.py --dataset image/medical
    python train_image.py --epochs 150 --lr 0.005
    python train_image.py --skip-pytorch                  # NumPy only
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train image classification models (NumPy + PyTorch)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python train_image.py                         # Default dataset + settings
  python train_image.py --dataset image/medical
  python train_image.py --epochs 150 --lr 0.005 --hidden 256 128
  python train_image.py --skip-pytorch          # NumPy model only
        """,
    )

    parser.add_argument(
        "--dataset", type=str, default="image/wildlife",
        help="Dataset key from registry (default: image/wildlife)",
    )
    ...
    parser.add_argument(
        ...
        help="Image resolution NxN (default: from registry config)",
    )
...
def stage_preprocess(args, config) -> dict:
    """
    Load or generate prepared data for the image dataset.

    Path 1 (fastest): Load pre-prepared data from disk
    Path 2: Run full pipeline: scan → preprocess → prepare
    """
    print("\n" + "=" * 70)
    ...
    # --- Full pipeline ---
    # Determine image directory from dataset key
    # e.g. "image/wildlife" → "data/image/wildlife"
    modality, subset = config.key.split("/")
    image_dir = Path("data") / modality / subset
    ...
    # Step A: Scan images
    print(f"\n📂 Scanning images from: {image_dir}")
    loader = ImageLoader(data_dir=str(image_dir), augment=False)

    # Step B: Preprocess to .npz
    print(f"\n🖼️  Preprocessing images to {target_shape[0]}×{target_shape[1]}...")
    preprocessor = ImagePreprocessor(target_shape=target_shape)

    spectrograms_dir = Path("data") / "preprocessed" / modality / subset
    preprocessor.generate_and_save_dataset(
        image_loader=loader,
...
def stage_generate_tsne(config, data: dict, net=None, pt_model=None):
    ...
    if pt_model is not None:
        ...
        with torch.no_grad():
            ...
            # network[:4] = Lin → ReLU → Lin → ReLU (penultimate features)
            n_modules = len(list(pt_model.network))
            penultimate_idx = n_modules - 1  # all but final Linear
...
def main():
    ...
    if config.modality != "image":
        print(f"❌ Dataset '{args.dataset}' is modality='{config.modality}', not 'image'.")
        print("   Use train.py / train_pytorch.py for audio datasets.")
        sys.exit(1)
    ...
    print("=" * 70)
    print(f"🖼️  IMAGE MODEL TRAINING — {config.name}")
    print("=" * 70)
    print(f"\n⚙️  Configuration:")
    ...
    print("\n" + "=" * 70)
    print("✅ IMAGE TRAINING PIPELINE COMPLETE")
    print("=" * 70)
    print(f"\n📊 Results:")
